...
import { ensureI18nSeeds, getMockI18nTranslationList, isoNow } from "~/utils/mock-data";
import { useResponseError, useResponseSuccess } from "~/utils/response";

/**
 * i18n_translation 批量操作。
 *
 * action:
 *   - "enable"  : 批量启用
 *   - "disable" : 批量禁用
...
export default defineEventHandler(async (event) => {
  ...
  if (targets.length === 0) {
    ...
    return useResponseError("NotFound", "no active i18n-translation found for given ids");
  }
